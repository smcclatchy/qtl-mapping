...
individual. In the figure below, $\mu$ equals `r round(c2eff[max_mkr, 4], 1)`, 
and $\beta$ equals `r round(abs(c2eff[max_mkr, 1]), 1)` for the alternative 
hypothesis (QTL exists).

![](../fig/nullvalt.png)

This linear model is <i>y</i> = `r round(c2eff[max_mkr,4], 1)` + 
`r round(abs(c2eff[max_mkr,1]), 1)`X + &epsilon;. The model intersects the 
genotype groups at their group means, and is based on &mu; and 
<i>&beta;<sub>k</sub></i> for chromosome 2 marker `r max_mkr` located at 
`r cross$pmap[['2']][max_mkr]` Mb.  

The effect of genotype `r dimnames(c2eff)[[2]][[3]]` (the &beta; for the 
`r dimnames(c2eff)[[2]][[3]]` genotype) at marker `r max_mkr` is 
`r round(c2eff[max_mkr, 3], 1)`, while the effect of the 
`r dimnames(c2eff)[[2]][[1]]` genotype is `r round(c2eff[max_mkr, 1], 1)` on 
the insulin phenotype. The effect of the `r dimnames(c2eff)[[2]][[2]]` genotype 
is `r round(c2eff[max_mkr,2], 1)` relative to $\mu$ equals 
`r round(c2eff[max_mkr,4], 1)`.
...
<<<<<<< HEAD
The result is a matrix of `r nrow(eff_chr7)` positions &times; `r ncol(eff_chr7)-1` 
genotypes. An additional column contains the intercept values (&mu;).
=======
The result is a matrix of `r nrow(eff_chr2)` positions $\times$  
`r ncol(eff_chr2)-1` genotypes. An additional column contains the intercept 
values ($\mu$).
>>>>>>> d2a8ee57371428b1b50d72156b7c910645d99dd1
...
`r dimnames(eff_chr7)[[2]][[2]]` genotype is centered around zero, with the
...
The position of the maximum LOD on chromosome 7 is `r subset(peaks, chr == '7')$pos`
...
```{r est_effects_insulin_c2_contr}
