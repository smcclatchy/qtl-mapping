library(qtl)
data(hyper)
x <- pull.geno(hyper)[,"D4Mit214"]
u <- runif(length(x), -0.075, 0.075)
y <- hyper$pheno[,1]

png(filename = "fig/hk-regress.png", width = 600, height = 600)
plot(y ~ x, type="n", xlab="Genotype", ylab="Phenotype",
     xlim=c(0.5, 2.5), xaxs="i", xaxt="n", main = "Haley-Knott Regression")
abline(v=1:2, lty=2, col="gray50")
points(x + 0.3 * u, y)
axis(side=1, at=1:2, labels=c("BB","BR"))
me <- tapply(y, x, mean)
segments((1:2)-0.15, me, (1:2)+0.15, me, lwd=3, col="orange1")
fakey <- rnorm(n=125, mean = mean(y), sd=sd(y))
points(1.5+(4*u[1:125]), fakey, col = "gray70")
segments((1.5)-0.15, mean(y), (1.5)+0.15, mean(y), lwd=3, col="orange1")
mtext("P(BB) = 1\nP(BR) = 0", side = 1, line = -1, at = .85)
mtext("P(BB) = 0\nP(BR) = 1", side = 1, line = -1, at = 2.15)
mtext("P(BB) = P(BR)\n      = 0.5", side = 1, line = 1, at = 1.5)
abline(v=1.5, lty=2, col="gray50")
segments(x0 = seq(from = 1, to = 2, by = 0.1), y0= 80, y1=83)
points(x = 1.4, y = 85, col = "green3", lwd=2)
segments(x0 = 1.27, y0= 85, x1=1.37, col = "green3", lwd=2)
segments(x0 = 1.37, y0= 85, x1=1.34, y1=86, col = "green3", lwd=2)
segments(x0 = 1.37, y0= 85, x1=1.34, y1=84, col = "green3", lwd=2)
abline(lm(y~x)$coef, col="blue", lwd=2)
dev.off()
