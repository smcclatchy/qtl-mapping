...
(QTL exists). This linear model is $y$ = 1 + 
...
sums the effects of all other genotypes except genotype k. For the leave one 
chromosome out (LOCO) method, $\beta_kX_k$ is the effect of genotypes on 
chromosome $k$, and $\beta_iX_i$  represents effect of genotypes on all other 
chromosomes.
